...
describe.runIf(shouldRun)("Postgres storage", () => {
    ...
    it("supports the core persistence flows on postgres", async () => {
        ...
        expect(ownerConversations.map((conversation) => conversation.id)).toEqual(expect.arrayContaining([friendship.conversationId, group.id]));
    ...
    it("allocates monotonic per-conversation message seq values under concurrency", async () => {
        ...
        await Promise.all(Array.from({ length: 20 }, (_, index) => server.sendAdminMessage({
        ...
        expect(messages.map((message) => message.seq)).toEqual(Array.from({ length: 20 }, (_, index) => index + 1));
    });
});
