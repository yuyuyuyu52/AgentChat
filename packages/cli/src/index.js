...
class AdminHttpClient {
    ...
    async listConversationMessages(conversationId, accountId, limit) {
        const search = new URLSearchParams({ accountId });
        if (limit) {
            search.set("limit", String(limit));
        }
        return this.request("GET", `/admin/conversations/${conversationId}/messages?${search.toString()}`);
    }
...
async function withAgentClient(flags, fn) {
...
export async function main(argv = process.argv.slice(2)) {
    ...
    const client = new AdminHttpClient(typeof flags.url === "string" ? flags.url : DEFAULT_HTTP_URL, typeof flags["admin-password"] === "string" ? flags["admin-password"] : undefined);
    ...
    if (scope === "user" && action === "create") {
        print(await client.createAccount(typeof flags.type === "string"
            ? {
    ...
    if (scope === "friend" && action === "add") {
        print(await client.createFriendship(requireString(flags, "from"), requireString(flags, "to")));
        return;
    }
    ...
    if (scope === "group" && action === "add-member") {
        print(await client.addGroupMember(requireString(flags, "group-id"), requireString(flags, "account")));
        return;
    }
    ...
    if (scope === "message" && action === "tail") {
        ...
        print(await client.listConversationMessages(requireString(flags, "conversation"), requireString(flags, "account"), limit));
        return;
    }
    if (scope === "agent") {
        const [agentScope, agentAction] = command.slice(1);
        if (agentScope === "friend" && agentAction === "add") {
            print(await withAgentClient(flags, async (client) => client.addFriend(requireString(flags, "peer"))));
            return;
        }
        ...
        if (agentScope === "friend" && agentAction === "requests") {
            print(await withAgentClient(flags, async (client) => client.listFriendRequests(typeof flags.direction === "string"
                ? flags.direction
                : "all")));
            return;
        }
        if (agentScope === "friend" && (agentAction === "accept" || agentAction === "reject")) {
            print(await withAgentClient(flags, async (client) => client.respondFriendRequest(requireString(flags, "request"), agentAction === "accept" ? "accept" : "reject")));
            return;
        }
        if (agentScope === "group" && agentAction === "create") {
            print(await withAgentClient(flags, async (client) => client.createGroup(requireString(flags, "title"))));
            return;
        }
        if (agentScope === "group" && agentAction === "add-member") {
            print(await withAgentClient(flags, async (client) => client.addGroupMember(requireString(flags, "group-id"), requireString(flags, "member"))));
            return;
        }
        ...
        if (agentScope === "conversation" && agentAction === "members") {
            print(await withAgentClient(flags, async (client) => client.listConversationMembers(requireString(flags, "conversation"))));
            return;
        }
        if (agentScope === "message" && agentAction === "send") {
            print(await withAgentClient(flags, async (client) => client.sendMessage(requireString(flags, "conversation"), requireString(flags, "body"))));
            return;
        }
        if (agentScope === "message" && agentAction === "tail") {
            const limit = typeof flags.limit === "string" ? Number(flags.limit) : undefined;
            print(await withAgentClient(flags, async (client) => client.listMessages(requireString(flags, "conversation"), limit ? { limit } : {})));
            return;
        }
        if (agentScope === "audit" && agentAction === "list") {
            const limit = typeof flags.limit === "string" ? Number(flags.limit) : undefined;
            const conversationId = typeof flags.conversation === "string" ? flags.conversation : undefined;
            print(await withAgentClient(flags, async (client) => client.listAuditLogs({
        ...
        if (agentScope === "plaza" && agentAction === "post") {
            print(await withAgentClient(flags, async (client) => client.createPlazaPost(requireString(flags, "body"))));
            return;
        }
        if (agentScope === "plaza" && agentAction === "list") {
            const limit = typeof flags.limit === "string" ? Number(flags.limit) : undefined;
            const authorAccountId = typeof flags.author === "string" ? flags.author : undefined;
            const beforeCreatedAt = typeof flags["before-created-at"] === "string" ? flags["before-created-at"] : undefined;
            const beforeId = typeof flags["before-id"] === "string" ? flags["before-id"] : undefined;
            print(await withAgentClient(flags, async (client) => client.listPlazaPosts({
        ...
        if (agentScope === "plaza" && agentAction === "get") {
            print(await withAgentClient(flags, async (client) => client.getPlazaPost(requireString(flags, "post"))));
            return;
        }
    }
