...
export class AgentChatServer {
    ...
    constructor(options = {}) {
        ...
        this.store = new AgentChatStore({
            databasePath: options.databasePath ?? new URL("../../data/agentchat.sqlite", import.meta.url).pathname,
    ...
    async createGroupAs(actorId, title) {
    ...
    async addGroupMember(conversationId, accountId) {
        ...
        for (const memberId of summary.memberIds) {
            if (memberId !== accountId) {
                this.dispatchEventToAccount(memberId, makeEvent("conversation.member_added", {
                    conversationId,
                    accountId,
    ...
    async listConversationMessages(accountId, conversationId, before, limit) {
        return this.store.listMessages(accountId, conversationId, before, limit);
    }
    async listOwnedConversationMessages(ownerSubject, conversationId, before, limit) {
    ...
    async handleHttpRequest(request, response) {
        try {
            ...
            if (method === "GET" && url.pathname === "/auth/google/callback") {
                ...
                this.startUserSession(response, profile.picture
                    ? {
                        createdAt: Date.now(),
                        subject: profile.sub,
                        email: profile.email,
            ...
            if (method === "GET" && url.pathname === "/auth/logout") {
                ...
                response.setHeader("set-cookie", this.makeSessionCookie("agentchat_user_session", "", { maxAge: 0 }));
                redirect(response, "/");
                return;
            }
            ...
            if (method === "POST" && url.pathname === "/admin/login") {
                ...
                response.setHeader("set-cookie", this.makeSessionCookie("agentchat_admin_session", sessionId, {
                    maxAge: 60 * 60 * 8,
                }));
            ...
            if (method === "POST" && url.pathname === "/admin/logout") {
                ...
                response.setHeader("set-cookie", this.makeSessionCookie("agentchat_admin_session", "", { maxAge: 0 }));
            ...
            if (method === "GET" && url.pathname === "/app/api/audit-logs") {
                const session = this.requireUserSession(request);
                const conversationId = typeof url.query.conversationId === "string" ? url.query.conversationId : undefined;
                const limit = typeof url.query.limit === "string" ? Number(url.query.limit) : undefined;
                jsonResponse(response, 200, await this.listOwnedAuditLogs(session.subject, {
                    ...(conversationId ? { conversationId } : {}),
                    ...(limit ? { limit } : {}),
                }));
                return;
            }
            const appConversationMessagesMatch = url.pathname?.match(/^\/app\/api\/conversations\/([^/]+)\/messages$/);
            if (method === "GET" && appConversationMessagesMatch) {
                const session = this.requireUserSession(request);
                const before = typeof url.query.before === "string" ? Number(url.query.before) : undefined;
                const limit = typeof url.query.limit === "string" ? Number(url.query.limit) : undefined;
                jsonResponse(response, 200, await this.listOwnedConversationMessages(session.subject, appConversationMessagesMatch[1], before, limit));
                return;
            }
            if (method === "POST" && url.pathname === "/app/api/accounts") {
                const session = this.requireUserSession(request);
                const body = CreateAccountBodySchema.parse(await readJson(request));
                jsonResponse(response, 201, await this.createAccount({
                    ...body,
                    owner: {
            ...
            if (method === "POST" && url.pathname === "/admin/messages") {
                ...
                if (!body.conversationId && !body.recipientId) {
                    throw new AppError("INVALID_ARGUMENT", "Either conversationId or recipientId is required");
                }
                jsonResponse(response, 201, await this.sendAdminMessage(body));
                return;
            }
            ...
            if (method === "GET" && messageMatch) {
                ...
                const before = typeof url.query.before === "string" ? Number(url.query.before) : undefined;
                const limit = typeof url.query.limit === "string" ? Number(url.query.limit) : undefined;
                jsonResponse(response, 200, await this.listConversationMessages(accountId, messageMatch[1], before, limit));
                return;
            }
    ...
    async handleSocketMessage(connection, rawMessage) {
        let requestId;
        try {
            ...
            switch (request.type) {
                case "connect": {
                    const account = await this.store.authenticateAccount(request.payload.accountId, request.payload.token);
                ...
                case "list_messages": {
                    const accountId = this.requireAuthenticated(connection);
                    this.sendResponse(connection, request.id, await this.store.listMessages(accountId, request.payload.conversationId, request.payload.before, request.payload.limit));
                    return;
                }
                case "send_message": {
                    const accountId = this.requireAuthenticated(connection);
                ...
                case "list_friend_requests": {
                    const accountId = this.requireAuthenticated(connection);
                    const result = await this.listFriendRequests(accountId, request.payload.direction ?? "all");
                    this.sendResponse(connection, request.id, result);
                    return;
                }
                case "respond_friend_request": {
                    const accountId = this.requireAuthenticated(connection);
                    const result = await this.respondFriendRequestAs(accountId, request.payload.requestId, request.payload.action);
                    this.sendResponse(connection, request.id, result);
                    return;
                }
                case "create_group": {
                ...
                case "add_group_member": {
                    const accountId = this.requireAuthenticated(connection);
                    const result = await this.addGroupMemberAs(accountId, request.payload.conversationId, request.payload.accountId);
                    this.sendResponse(connection, request.id, result);
                    return;
                }
                case "list_conversation_members": {
                    const accountId = this.requireAuthenticated(connection);
                    const result = await this.listConversationMembers(accountId, request.payload.conversationId);
                    this.sendResponse(connection, request.id, result);
                    return;
                }
                ...
                case "subscribe_plaza": {
                    ...
                    this.sendResponse(connection, request.id, await this.listPlazaPosts({
                        ...(request.payload?.limit ? { limit: request.payload.limit } : {}),
                    }));
                    return;
                }
                ...
                case "create_plaza_post": {
                    const accountId = this.requireAuthenticated(connection);
                    this.sendResponse(connection, request.id, await this.createPlazaPost(accountId, request.payload.body));
                    return;
                }
            }
        }
            ...
            const appError = error instanceof z.ZodError
                ? new AppError("INVALID_ARGUMENT", error.message)
                : asAppError(error);
            connection.socket.send(JSON.stringify(makeErrorFrame(appError.code, appError.message, requestId)));
        }
    }
    async handleSocketClose(connection) {
    ...
    async broadcastPresence(accountId, status) {
        ...
        for (const watcherId of watcherIds) {
            this.dispatchEventToAccount(watcherId, event, (connection) => connection.subscribedConversationFeed);
        }
    }
    async broadcastConversationCreated(accountId, conversationId) {
        const summary = await this.store.getConversationSummaryForAccount(accountId, conversationId);
        this.dispatchEventToAccount(accountId, makeEvent("conversation.created", summary), (connection) => connection.subscribedConversationFeed);
    }
    async broadcastMessage(message) {
        ...
        for (const memberId of memberIds) {
            this.dispatchEventToAccount(memberId, event, (connection) => connection.subscribedConversationIds.has(message.conversationId));
        }
    }
    broadcastPlazaPostCreated(post) {
    ...
    dispatchEventToAccount(accountId, event, predicate) {
    ...
    sendResponse(connection, requestId, payload) {
        connection.socket.send(JSON.stringify(makeResponse(requestId, payload)));
    }
    requireAuthenticated(connection) {
    ...
    startUserSession(response, session) {
        const sessionId = randomUUID();
        this.userSessions.set(sessionId, session);
        response.setHeader("set-cookie", this.makeSessionCookie("agentchat_user_session", sessionId, {
            maxAge: 60 * 60 * 24 * 7,
        }));
    }
    ...
    makeSessionCookie(name, value, options) {
        return [
            `${name}=${value}`,
            "Path=/",
            "HttpOnly",
