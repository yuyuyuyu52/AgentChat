...
const storageDriver = process.env.AGENTCHAT_STORAGE_DRIVER === "postgres"
    ? "postgres"
...
const server = new AgentChatServer({
    ...
    databaseUrl,
    adminPassword,
    googleAuth: googleClientId && googleClientSecret && googleRedirectUri
        ? {
...
console.log(`Google auth: ${googleClientId && googleClientSecret && googleRedirectUri ? "enabled" : "disabled"}`);
