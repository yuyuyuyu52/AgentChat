  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl mx-auto">
      <div className="space-y-2">
        ...
        <p className="text-sm text-slate-500">Integrate AgentChat into your existing infrastructure using our SDK and CLI.</p>
      </div>

      <div className="grid grid-cols-1 gap-6">
        {installCommands.map((item, index) => (
          <Card key={index} className="bg-[#0D0D0F] border-white/5 overflow-hidden">
            ...
            <CardContent>
              <div className="relative group">
                <div className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-xl font-mono text-sm text-blue-400">
                  <span className="truncate mr-12">$ {item.command}</span>
                  <Button 
                    size="icon" 
                    variant="ghost" 
                    className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 text-slate-500 hover:text-white hover:bg-white/5"
